        ...
        for (ArrayList<String> als : data) {
            ...
            while (iterator < als.size()){
                ...
                if (als.get(iterator + 1).charAt(0) == 'E'){ // GESTIONS DES ESPECES REMARCABLES
                    ...
                    requete = "INSERT INTO espece (id, nom_esp, photo, GPS_lat, GPS_long, GPS_lat_lam, GPS_long_lam, date_enr, heure_enr) VALUES(uuid(), '" + als.get(iterator) + "', '" + als.get(iterator+1) +  "', " + als.get(iterator+2) + ", " + als.get(iterator+3) + ", " + als.get(iterator+4) + ", " + als.get(iterator+5) + ", '" + date + "', '" + heure + "');";
                    System.out.println(requete);
